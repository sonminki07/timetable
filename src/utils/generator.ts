  ...
  function combine(
    ...
    if (groupIndex === filteredGroups.length) {
      ...
      if (hasHardDayOff) {
        ...
        const enrichedLectures = currentLectures.map(l => ({
          ...l,
          timeBlocks: l.timeBlocks.map((block: TimeBlock) => {
            const dayBlocks = currentLectures
              .flatMap(otherL => otherL.timeBlocks.map((b: TimeBlock) => ({ ...b, title: otherL.title })))
              .filter((b: any) => b.day === block.day)
              .sort((a, b) => a.start - b.start);
            
            const currentIndex = dayBlocks.findIndex((b: any) => b.start === block.start && b.title === l.title);
            let moveInfo = null;
            if (currentIndex > 0) {
    ...
    for (const course of currentGroup.courses) {
      ...
      const overlappingCourse = currentLectures.find((c) =>
        course.allTimes.some((t: string) => c.allTimes.includes(t))
      );
